 ...
 * Sat Feb 20 2021 13:32:36 GMT+0400 (GMT+04:00)
...
this.procedures["loadScene"] = loadScene;
this.procedures["getVideoName"] = getVideoName;
this.procedures["getFileName"] = getFileName;
...
var scenes, sceneName, oldScene, infos, activeScene, currentVideoName;
...
function loadScene(sceneName) {
  setHTMLElemStyle('display', 'flex', 'preloader', false);
  unloadScene(getFileName(oldScene));
  appendScene(getFileName(sceneName), getFileName(sceneName), true, false, function() {
    setHTMLElemStyle('display', 'none', 'preloader', false);
    setActiveCamera('Camera');
    registerOnClick(dictGet(scenes, activeScene), false, false, [0,1,2], function() {
      oldScene = activeScene;
      loadScene(_pGlob.pickedObject);
      activeScene = _pGlob.pickedObject;
    }, function() {});
    registerOnClick(dictGet(infos, activeScene), false, false, [0,1,2], function() {
      changeVis(dictGet(infos, activeScene), false);
      changeVis('tv', true);
      makeParent('tv', 'Camera');
      replaceTexture('tv_video', 'video_screen.jpg', getVideoName(_pGlob.pickedObject), function() {
        playSound(getVideoName(_pGlob.pickedObject), false);
      });
      currentVideoName = _pGlob.pickedObject;
    }, function() {
      stopSound(getVideoName(currentVideoName));
      changeVis('tv', false);
      changeVis(dictGet(infos, activeScene), true);
    });
    registerOnHover(dictGet(scenes, activeScene), false, function() {
      outline(_pGlob.hoveredObject, 'ENABLE');
    }, function() {
      outline(_pGlob.hoveredObject, 'DISABLE');
    });
    registerOnHover(dictGet(infos, activeScene), false, function() {
      outline(_pGlob.hoveredObject, 'ENABLE');
    }, function() {
      outline(_pGlob.hoveredObject, 'DISABLE');
    });
  }, function() {
    setHTMLElemAttribute('innerHTML', ['Пожалуйста, подождите немного...',Math.round(_pGlob.percentage),'%'].join(''), 'preloader_text', false);
  }, function() {});
}
...
// Describe this function...
function getVideoName(sceneName) {
  return loadVideo(['./assets/',sceneName,'.mp4'].join(''));
}

// Describe this function...
function getFileName(sceneName) {
  return ['./assets/',sceneName,'.gltf'].join('');
}


scenes = {};
dictSet(scenes, 'scene1', ['scene2', 'scene6']);
dictSet(scenes, 'scene2', ['scene1', 'scene5', 'scene6', 'scene7']);
dictSet(scenes, 'scene3', ['scene2', 'scene4', 'scene5']);
dictSet(scenes, 'scene4', ['scene3', 'scene5', 'scene6']);
dictSet(scenes, 'scene5', ['scene3', 'scene4', 'scene6']);
dictSet(scenes, 'scene6', ['scene1', 'scene2', 'scene5', 'scene7']);
dictSet(scenes, 'scene7', ['scene6']);
infos = {};
dictSet(infos, 'scene1', ['borodinskoe1']);
dictSet(infos, 'scene2', ['kyrskai2']);
dictSet(infos, 'scene3', ['vperednazapad3', 'ykrutieleningrada3']);
dictSet(infos, 'scene4', ['bitvazaberlin4']);
dictSet(infos, 'scene7', ['vitrinarubega7', 'letchiki7']);
activeScene = 'scene1';
oldScene = activeScene;
